...
class WritingToolApp(QtWidgets.QApplication):
    ...
    def __init__(self, argv):
        ...
        self.output_ready_signal.connect(self.queue_text_replacement)
    ...
    def queue_text_replacement(self, new_text):
        """
    ...
    def replace_text(self, force=False):
        """
        Replace the selected text with the new text generated by the AI.
        """

        #If it's been less than 0.1 seconds since the last run, don't run unless force is true
        if time.time() - self.last_replace < 0.1 and not force:
            return

        self.last_replace = time.time()
