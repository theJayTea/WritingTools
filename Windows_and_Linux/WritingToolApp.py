...
class WritingToolApp(QtWidgets.QApplication):
    ...
    def _get_config_dir(self):
        """Returns the persistent, user-specific config directory."""
        # Determine the Windows_and_Linux directory correctly for both script and exe
        if getattr(sys, "frozen", False):
            # Running as exe - sys.argv[0] points to the exe in dist/
            app_dir = os.path.dirname(os.path.dirname(sys.argv[0]))
        else:
            # Running as script - __file__ is in Windows_and_Linux/
    ...
    def _get_dist_dir(self):
        """Returns the dist directory for builds."""
        # Determine the Windows_and_Linux directory correctly for both script and exe
        if getattr(sys, "frozen", False):
            # Running as exe - sys.argv[0] points to the exe in dist/
            app_dir = os.path.dirname(os.path.dirname(sys.argv[0]))
    ...
    def _ensure_directories_exist(self):
        """Create config and dist directories if they don't exist."""
        ...
        logging.debug(f"Ensured directories exist: {config_dir}, {dist_dir}")

    def load_config(self):
        """
        Load the configuration file from the local 'config' directory.
        """
        config_dir = self._get_config_dir()
        os.makedirs(config_dir, exist_ok=True)
    ...
    def load_options(self):
        """
        Load the options file with priority order:
        1. config/options.json (user-specific)
        2. dist/options.json (build-specific)
        3. default-options.json (default template)
        """
        config_dir = self._get_config_dir()
        dist_dir = self._get_dist_dir()
        ...
        # Priority paths
        config_options_path = os.path.join(config_dir, "options.json")
        dist_options_path = os.path.join(dist_dir, "options.json")
        default_options_path = os.path.join(app_dir, "default-options.json")

        # Determine which file to use
        if os.path.exists(config_options_path):
            self.options_path = config_options_path
            logging.debug(f"Using config options: {self.options_path}")
        elif os.path.exists(dist_options_path):
            self.options_path = dist_options_path
            logging.debug(f"Using dist options: {self.options_path}")
        elif os.path.exists(default_options_path):
            self.options_path = default_options_path
            logging.debug(f"Using default options: {self.options_path}")
        else:
            logging.error("No options file found in any location")
            self.options = None
            return
        ...
        try:
            ...
            logging.error(f"Error loading options from {self.options_path}: {e}")
            self.options = None

    def save_config(self, config):
        """
        Save the configuration file to the local 'config' directory.
        """
