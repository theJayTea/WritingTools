...
class SettingsWindow(QtWidgets.QWidget):
    ...
    def init_provider_ui(self, provider: AIProvider, layout):
        ...
        for setting in provider.settings:
            setting.set_value(
                self.app.config["providers"][provider.provider_name].get(
                    setting.name, setting.default_value
                )
            )
    ...
    def init_ui(self):
        ...
        current_provider = self.app.config.get(
            "provider", self.app.providers[0].provider_name
        )
        ...
        bottom_layout.setContentsMargins(
            30, 0, 30, 30
        )  # Match content margins except top
        bottom_layout.setSpacing(10)

        # Add save button to bottom container
        save_button = QtWidgets.QPushButton(
            _("Finish AI Setup") if self.providers_only else _("Save")
        )
        ...
        if not self.providers_only:
            ...
            restart_notice.setStyleSheet(
                f"font-size: 15px; color: {'#cccccc' if colorMode == 'dark' else '#555555'}; font-style: italic;"
            )
        ...
        self.resize(
            592, desired_height
        )  # Use an exact width of 592px so stuff looks good!
    ...
    def save_settings(self):
        ...
        if not self.providers_only:
            ...
            self.app.config["theme"] = (
                "gradient" if self.gradient_radio.isChecked() else "plain"
            )
        else:
