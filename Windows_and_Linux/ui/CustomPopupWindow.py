...
class CustomPopupWindow(QtWidgets.QWidget):
    ...
    @staticmethod
    def load_options():
        """Load options with priority: config/ > dist/ > template/"""
        # Determine the Windows_and_Linux directory
        if getattr(sys, "frozen", False):
            # Running as exe - sys.argv[0] points to the exe in dist/
            app_dir = os.path.dirname(os.path.dirname(sys.argv[0]))
        else:
            # Running as script - find Windows_and_Linux directory
            current_file = os.path.abspath(__file__)
            app_dir = os.path.dirname(os.path.dirname(current_file))

        # Priority paths
        config_path = os.path.join(app_dir, "config", "options.json")
        dist_path = os.path.join(app_dir, "dist", "options.json")
        default_path = os.path.join(app_dir, "default-options.json")

        # Try each path in order
        for path in [config_path, dist_path, default_path]:
            if os.path.exists(path):
                try:
                    with open(path, "r") as f:
                        data = json.load(f)
                        logging.debug(f"Options loaded from: {path}")
                        return data
                except Exception as e:
                    logging.error(f"Error loading options from {path}: {e}")
                    continue
    ...
    def save_options(options):
        """Save options according to execution context:
        - Dev script: config/ only
        - Build exe: config/ + dist/
        """
        # Determine the Windows_and_Linux directory
        if getattr(sys, "frozen", False):
            # Running as exe - sys.argv[0] points to the exe in dist/
        ...
        config_path = os.path.join(config_dir, "options.json")
        try:
            with open(config_path, "w") as f:
                json.dump(options, f, indent=2)
            logging.debug(f"Options saved to config: {config_path}")
        except Exception as e:
            logging.error(f"Error saving options to config: {e}")
            return

        # If running as exe, also save to dist/
        if is_exe:
            dist_dir = os.path.join(app_dir, "dist")
            os.makedirs(dist_dir, exist_ok=True)
            dist_path = os.path.join(dist_dir, "options.json")
            try:
                with open(dist_path, "w") as f:
                    json.dump(options, f, indent=2)
                logging.debug(f"Options also saved to dist: {dist_path}")
            except Exception as e:
                logging.error(f"Error saving options to dist: {e}")
        else:
            logging.debug("Running as script - options saved to config/ only")
    ...
    def add_edit_delete_icons(self, btn):
        ...
        del_icon = os.path.join(
            os.path.dirname(sys.argv[0]),
            "icons",
            "cross" + ("_dark" if colorMode == "dark" else "_light") + ".png",
        )
    ...
    def on_reset_clicked(self):
        ...
        confirm_box.setText(
            "To reset the buttons to their original configuration, Writing Tools would need to quit, so you'd need to relaunch Writing Tools.exe.\nAre you sure you want to continue?"
        )
    ...
    def delete_button_clicked(self, btn):
        ...
        confirm.setText(
            f"To delete the '{key}' button, Writing Tools would need to quit, so you'd need to relaunch Writing Tools.exe.\nAre you sure you want to continue?"
        )
